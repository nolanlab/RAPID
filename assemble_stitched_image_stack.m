...
function I = assemble_stitched_image_stack(source_directory, img_name_grid, global_y_img_pos, global_x_img_pos, nCh, fusion_method, alpha, tile_weights)
...
switch fusion_method
    case 'overlay'
        I = zeros(stitched_img_height, stitched_img_width, nCh, class_str);
        ...
        for ch = 1:nCh
            for k = 1:nb_img_tiles
                ...
                if ~isempty(img_name_grid{img_idx})
                    ...
                    current_image = double(imread([source_directory, img_name_grid{img_idx}],ch));
                    if ~isempty(current_image)
                        ...
                        I(y_st:y_end,x_st:x_end,ch) = current_image;
                    end
                end
            end
        end
    ...
    case 'linear'
        I = zeros(stitched_img_height, stitched_img_width, nCh, 'single');
        ...
        countsI = zeros(stitched_img_height, stitched_img_width, nCh, 'single');
        % Assemble images
        for ch = 1:nCh
            for k = 1:nb_img_tiles
                % Read the current image
                img_idx = assemble_ordering(k);
                if ~isempty(img_name_grid{img_idx})
                    current_image = single(imread([source_directory, img_name_grid{img_idx}],ch));
                    if ~isempty(current_image)
                        ...
                        y_st = global_y_img_pos(img_idx);
                        y_end = global_y_img_pos(img_idx)+img_height-1;
                        I(y_st:y_end,x_st:x_end,ch) = I(y_st:y_end,x_st:x_end,ch) + current_image;
                        countsI(y_st:y_end,x_st:x_end,ch) = countsI(y_st:y_end,x_st:x_end,ch) + w_mat;
                    end
                end
            end
        end
        I = I./countsI;
        I(isnan(I))=0; % if I is NaN, set it to zeros
        I = cast(I,class_str);
